...
$(document).on("click", "#bFollow", function () {
  var query = {
...
function getMyPageNav(nav) {
  ...
  switch (nav) {
    case 1:
      ...
      break;
    case 2:
      sessionStorage.setItem("nav", "save");
      setNav(nav);
      unSetNav(nav);
      break;
    case 3:
...
function addListMyPage(data) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    innerList +=
      '          <div class="writer" id="writer_' +
      list[i].bno +
      '"><a href="/member/myPage?num=1&userID=' +
      $("#userID").val() +
      '"><img alt="" src=' +
      $(".ff > img").attr("src") +
      "></div><a>";
    ...
    innerList +=
      ...
      "</div>";
    innerList +=
      '            <div id="viewCnt">조회수 ' + list[i].viewCnt + "회</div>";
    innerList += "         </div>";
    innerList += "        </li>";
    innerList += "      </ul>";
...
function addListMyFforf(data) {
  for (var i = 0; i < member.length; i++) {
    var innerList;
    innerList =
      '<div class="neighbor"><div class="pro"><a href="/member/myPage?num=1&userID=' +
      member[i].id +
...
function addListMyFol(data) {
  for (var i = 0; i < member.length; i++) {
    ...
    innerList =
      '<div id="member_' +
      i +
      '" class="nplus"><input type="hidden" id="folUserID_' +
      i +
      '" value="' +
      member[i].id +
      '"><div class="prof"><a href="/member/myPage?num=1&userID=' +
      member[i].id +
      '"><img width="100" height="100" alt="" src=' +
      member[i].mImg +
      ...
      member[i].mNickname +
      '님</div><p>회원님에게 이웃 요청을 보냈습니다</p><div class="check"><button class="yes" type="button" onclick="follow(' +
      i +
      ')">수락</button><button class="no" type="button" onclick="unFollow(' +
      i +
      ')">거절</button></div></div>';
    $(".in").append(innerList);
  }
...
function getTimeStamp(time) {
  ...
  if (sessionStorage.getItem("nav") == "list") {
    ...
    if ((curTime.getTime() - d.getTime()) / (1000 * 60 * 60 * 24) < 1) {
      ...
      return (
        Math.floor((curTime.getTime() - d.getTime()) / (1000 * 60 * 60 * 24)) +
        "일전"
      );
    } else {
      return (
        Math.floor(
          (curTime.getTime() - d.getTime()) / (1000 * 60 * 60 * 24) / 7
        ) + "주전"
      );
    }
  }
